...
def get_GA_data_daily(required_date,file):
    mydb = MySQLdb.connect(host='172.16.1.73', user='prasanjit', passwd='prasan123', db='carwaledata')
    cursor = mydb.cursor()
    ...
    cursor.execute(""" SELECT ModelId,ModelMaskingName,MakeMaskingName
                ...
                (SELECT ModelId FROM GA_WebsiteDaily WHERE date = %s)""",(required_date))
    ...
    if masking_names is not None:
        ...
        for masking_name in masking_names: 
            try:
                ...
                data = get_data(service, '529535', required_date, required_date, segment)
                data3 = get_data2(service, '529535', required_date, required_date, filters)

                sql = """insert into GA_WebsiteDaily values(%s,%s,%s,%s,%s)"""
                args = [masking_name[0], required_date, data3['ga:pageViews'], data['ga:users'], data['ga:sessions']]
...
def convert_daily_data_to_csv(csvFile,file,required_date):
    mydb = MySQLdb.connect(host='172.16.1.73', user='prasanjit', passwd='prasan123', db='carwaledata')
    cursor = mydb.cursor()
    
    cursor.execute("""SELECT ModelId,Date,Pageviews,Users,Sessions FROM carwaledata.GA_WebsiteDaily
                    where date = %s""",required_date)
    records = cursor.fetchall()
    if records is not None:
        file.write("\n"+"Writing data into CSV file for : START :"+str(required_date))
        with open(csvFile, "w") as output:
            writer = csv.writer(output, lineterminator='\n')
            writer.writerows(records)
            subprocess.call(['chmod', '0777', csvFile])
    file.write("\n"+"Writing : END : "+str(required_date))
...
def main():
    ...
    logFile = '../GAUpdationLogs/GA_data_'+required_date+'.log'
    csvFile = '../GA_CSV_data/GA_data_'+required_date+'.csv'

    with io.FileIO(logFile, "w") as file:
        file.write("Date : "+required_date)
    ...
    get_GA_data_daily(required_date,file)
    
#    convert_daily_data_to_csv(csvFile,file,required_date)

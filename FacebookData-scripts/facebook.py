...
def getFacebookPageDailyData(required_date):
	...
	for pageId in pageIds:
		pageData = get_page_data(str(pageId[1]),token,required_date)
		postCount = 0
		...
		if pageData:
			for data in pageData["posts"]["data"]:
				...
				if check_post_id:
					...
					args2 = (post_id,data["created_time"][:10],pageId[1],message,LikesCount,sharesCount,picture,full_picture)
